...
import { Plus, Trash2, X, ChevronDown, ChevronUp, LogOut, Shield, TrendingUp, Search, ArrowLeft, Wallet, User, Save, CheckCircle, Clock, Link } from "lucide-react"
...
export default function App() {
  ...
  const saveProfile = async () => {
    ...
    try {
      const { error } = await supabase.from("profiles").update({
        ...
        profile_link: profileForm.profile_link.trim()
  ...
  const delEntry = async (entryId) => { await supabase.from("view_entries").delete().eq("id",entryId); await fetchPosts(session?.user?.id) }
  ...
  const executePayout = async (user) => {
    ...
    try {
      ...
      setPayoutNote("")
      await loadAllUsers()
      // Reload selUser mit aktualisierten Daten
      setSelUser(null)
  ...
  const totals = useMemo(() => {
    ...
    const openEarnings = calcOpenEarnings(posts, lastPayoutDate)
        ...
        {authMode==="signup"&&(
          <>
            ...
            <div style={{ position:"relative" }}>
              ...
              <input style={{ ...INPUT, paddingLeft:32 }} placeholder="TikTok / Instagram Profillink" value={form.profile_link} onChange={(e)=>setForm((f)=>({...f,profile_link:e.target.value}))}/>
            </div>
            <div style={{ position:"relative" }}>
    ...
    return (
      <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:60 }}>
        ...
        <div style={{ padding:"0 18px", height:58, borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", gap:12, position:"sticky", top:0, background:C.bg+"F0", backdropFilter:"blur(12px)", zIndex:30 }}>
          ...
          {u.is_admin&&<span style={{ fontSize:10, color:C.gold, fontWeight:700, letterSpacing:"0.08em", display:"flex", alignItems:"center", gap:3 }}><Shield size={10}/>Admin</span>}
          <span style={{ marginLeft:"auto", fontSize:10, color:C.gold, fontWeight:700, display:"flex", alignItems:"center", gap:4 }}><Shield size={10}/>Admin-Ansicht</span>
        </div>
        ...
        <div style={{ padding:"24px 18px 0" }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:8, display:"flex", alignItems:"center", gap:5 }}><Wallet size={11}/> Wallet</div>
            <div style={{ fontSize:13, color:C.white, wordBreak:"break-all", fontFamily:"monospace", marginBottom:10 }}>{u.wallet||"—"}</div>
            ...
            {u.profile_link
              ? <a href={u.profile_link} target="_blank" rel="noreferrer" style={{ fontSize:13, color:C.blueGlow, wordBreak:"break-all" }}>{u.profile_link}</a>
              : <div style={{ fontSize:13, color:C.silver }}>—</div>
            }
          </div>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
            ...
            {u.lastPayout
              ? <div><div style={{ fontSize:16, fontWeight:700, color:C.green }}>{eur(u.lastPayout.amount)}</div><div style={{ fontSize:12, color:C.silver, marginTop:3 }}>{fmtDate(u.lastPayout.paid_at)}{u.lastPayout.note&&` · ${u.lastPayout.note}`}</div></div>
          ...
          {u.openEarnings > 0 && (
            <div style={{ background:C.card, border:`1px solid ${C.green}40`, borderRadius:12, padding:"16px", marginBottom:12 }}>
              ...
              <button
                onClick={()=>executePayout(u)}
                disabled={payoutBusy}
                style={{ width:"100%", background:payoutBusy?C.surface:`linear-gradient(135deg,#1a7a4a,${C.green})`, color:payoutBusy?C.silver:"#0B0B0F", border:"none", borderRadius:10, fontWeight:800, fontSize:14, padding:"13px 0", cursor:payoutBusy?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}
              >
          ...
          {/* Posts */}
          <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:10 }}>Posts & Views</div>
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:100 }}>
      ...
      {!isAdmin&&tab==="posts"&&(
        <div style={{ padding:"28px 18px 16px" }}>
          ...
          <div style={{ fontSize:13, color:C.silver, marginTop:4 }}>Gesamt: {eur(totals.gE)} · {fmt(totals.gV)} Views</div>
