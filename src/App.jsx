...
// Pending + Approved zählen (nur Rejected nicht)
const activePosts = (posts) => (posts||[]).filter((p)=>p.status!=="rejected")
const earningsPosts = activePosts
const approvedPosts = activePosts
...
export default function App() {
  ...
  const refreshSelUser = async () => {
    ...
    ud.posts?.forEach((p)=>{ const lv=latestV(p); v+=lv; byP[p.platform]=(byP[p.platform]||0)+lv })
    const userPayouts=pd||[]; const lastPayout=userPayouts[0]||null
    const lastPayoutDate=lastPayout?.paid_at?.slice(0,10)||null
    const totalEarnings=calcTotalEarnings(ud.posts); const openEarnings=calcOpenEarnings(ud.posts,lastPayoutDate)
    const totalPaid=userPayouts.reduce((s,p)=>s+p.amount,0)
    const pendingCount=(ud.posts||[]).filter((p)=>p.status==="pending").length
    setSelUser({ ...ud, views:v, byP, totalEarnings, openEarnings, totalPaid, lastPayout, userPayouts, pendingCount })
  }
  ...
  const calcTotalEarnings = (userPosts) => { let t=0; earningsPosts(userPosts).forEach((p)=>{ t+=payout(latestV(p)) }); return t }
