...
// Pending + Approved zählen für Anzeige (Views + Geld)
const activePosts   = (posts) => (posts||[]).filter((p)=>p.status!=="rejected")
// Nur Approved für den Payout-Button
const earningsPosts = (posts) => (posts||[]).filter((p)=>p.status==="approved")
...
export default function App() {
  ...
  const fetchProfile = async (userId) => {
    ...
    // Load profile links
    const { data: links } = await supabase.from("profile_links").select("*").eq("user_id", userId).order("created_at")
    setProfileLinks(links||[])
    setLoading(false); setScreen("app"); setTab("dashboard")
    fetchPosts(userId, data?.is_admin)
    if (data?.is_admin) loadAllUsers()
    else fetchMyPayouts(userId)
  }
  ...
  const deleteAccount = async () => {
    if (!deletePassword) { setDeleteErr("Bitte Passwort eingeben."); return }
    ...
    try {
      ...
      if (authErr) { setDeleteErr("Falsches Passwort."); return }
      // Account löschen
      const { error } = await supabase.rpc("delete_own_account")
      if (error) { setDeleteErr("Fehler beim Löschen: "+error.message); return }
      await supabase.auth.signOut()
    } finally { setDeleteBusy(false) }
  }

  const addProfileLink = async () => {
    if (!newLinkUrl.trim()) { setLinkMsg("Bitte Link eingeben."); return }
    setLinkMsg("")
    const { error } = await supabase.from("profile_links").insert({ user_id:session.user.id, platform:newLinkPlatform, url:newLinkUrl.trim() })
    if (error) { setLinkMsg("Fehler: "+error.message); return }
    const { data: links } = await supabase.from("profile_links").select("*").eq("user_id", session.user.id).order("created_at")
    setProfileLinks(links||[])
    setNewLinkUrl(""); setLinkMsg("Link hinzugefügt ✓")
  }

  const delProfileLink = async (id) => {
    await supabase.from("profile_links").delete().eq("id", id)
    setProfileLinks((l)=>l.filter((x)=>x.id!==id))
  }
  ...
  const handleAuth = async () => {
    ...
    try {
      if (authMode==="signup") {
        if (!form.username.trim()||!form.wallet.trim()) { setAuthErr("Bitte Benutzername und Wallet ausfüllen."); return }
        const { data, error } = await supabase.auth.signUp({ email:form.email, password:form.password })
        if (error) { setAuthErr(error.message); return }
        if (data.user) {
          // Prüfen ob E-Mail in Admin-Whitelist
          const { data: adminCheck } = await supabase.from("admin_emails").select("email").eq("email", form.email.trim().toLowerCase()).single()
          const isAdminEmail = !!adminCheck
          ...
          if (profileErr) {
            if (profileErr.code==="23505") { setAuthErr("Dieser Benutzername ist bereits vergeben."); return }
  ...
  const saveProfile = async () => {
    ...
    try {
      const { error } = await supabase.from("profiles").update({
        ...
        wallet: profileForm.wallet.trim()
  ...
  const addPost = async () => {
    ...
    if (!profile?.is_admin && profileLinks.length===0) { setDraftErr("Bitte zuerst mindestens einen Profillink unter Profil eintragen."); return }
    setBusy(true)
  ...
  const addEntry = async (postId) => {
    ...
    setPosts((prev)=>prev.map((p)=>{
      ...
      const rest    = (p.view_entries||[]).filter((h)=>h.date!==date)
      const entries = [...rest, { id:"tmp-"+date, date, views }].sort((a,b)=>a.date.localeCompare(b.date))
      return { ...p, view_entries: entries }
    }))
  ...
  const refreshSelUser = async () => {
    if (!selUser) return
    const { data: ud } = await supabase.from("profiles").select("*, posts(*, view_entries(*))").eq("id", selUser.id).single()
    const { data: pd } = await supabase.from("payouts").select("*").eq("user_id", selUser.id).order("paid_at", { ascending:false })
    const { data: links } = await supabase.from("profile_links").select("*").eq("user_id", selUser.id).order("created_at")
    if (!ud) return
    let v=0; const byP={ tiktok:0, instagram:0, youtube:0 }
    activePosts(ud.posts).forEach((p)=>{ const lv=latestV(p); v+=lv; byP[p.platform]=(byP[p.platform]||0)+lv })
    const userPayouts=pd||[]; const lastPayout=userPayouts[0]||null
    const lastPayoutDate=lastPayout?.paid_at?.slice(0,10)||null
    const totalEarnings=calcTotalEarnings(ud.posts)
    const openEarnings=calcOpenEarnings(ud.posts,lastPayoutDate)
    const payoutAmount=calcPayoutAmount(ud.posts,lastPayoutDate)
    const totalPaid=userPayouts.reduce((s,p)=>s+p.amount,0)
    const pendingCount=(ud.posts||[]).filter((p)=>p.status==="pending").length
    setSelUser({ ...ud, views:v, byP, totalEarnings, openEarnings, payoutAmount, totalPaid, lastPayout, userPayouts, pendingCount, profileLinks:links||[] })
  }
  ...
  const setPostStatus = async (postId, status) => {
    ...
    if (selUser) setSelUser((u)=>({ ...u, posts:(u.posts||[]).map((p)=>p.id===postId?{...p,status}:p) }))
    await supabase.from("posts").update({ status }).eq("id", postId)
    setStatusBusy(null)
    await refreshAll()
    await refreshSelUser()
  }
  ...
  const delPost = async (id) => {
    ...
    if (selUser) setSelUser((u)=>({ ...u, posts:(u.posts||[]).filter((p)=>p.id!==id) }))
  ...
  const calcOpenEarnings = (userPosts, lastPayoutDate) => {
    let total = 0
    activePosts(userPosts).forEach((p) => {
  ...
  const calcTotalEarnings = (userPosts) => { let t=0; activePosts(userPosts).forEach((p)=>{ t+=payout(latestV(p)) }); return t }

  // Nur für den Payout-Button: nur approved Posts
  const calcPayoutAmount = (userPosts, lastPayoutDate) => {
    let total = 0
    earningsPosts(userPosts).forEach((p) => {
      const entries = [...(p.view_entries||[])].sort((a,b)=>a.date.localeCompare(b.date))
      const currentViews = entries.length ? entries.at(-1).views : 0
      const baseViews = lastPayoutDate ? (entries.filter((e)=>e.date<=lastPayoutDate).at(-1)?.views||0) : 0
      total += Math.max(0, payout(currentViews)-payout(baseViews))
  ...
  const myActivePosts = useMemo(() => myPosts.filter((p)=>p.status!=="rejected"), [myPosts])
  ...
  const userStats = useMemo(() => {
    ...
    const totalViews    = myActivePosts.reduce((s,p)=>s+latestV(p),0)
    const over3k        = myActivePosts.filter((p)=>latestV(p)>=3000).length
    const byP           = { tiktok:0, instagram:0 }
    myActivePosts.forEach((p)=>{ byP[p.platform]=(byP[p.platform]||0)+latestV(p) })
  ...
  }, [myPosts, myActivePosts, myPayouts])
  ...
  const loadBoard = async () => {
    ...
    const rows = (data||[]).map((u) => {
      let v=0,e=0
      activePosts(u.posts).forEach((p)=>{ const lv=latestV(p); v+=lv; e+=payout(lv) })
  ...
  const loadAllUsers = async () => {
    ...
    const { data: linksData }   = await supabase.from("profile_links").select("*")
    const rows = (usersData||[]).map((u) => {
      let v=0; const byP={ tiktok:0, instagram:0, youtube:0 }
      activePosts(u.posts).forEach((p)=>{ const lv=latestV(p); v+=lv; byP[p.platform]=(byP[p.platform]||0)+lv })
      ...
      const payoutAmount  = calcPayoutAmount(u.posts, lastPayoutDate)
      const totalPaid     = userPayouts.reduce((s,p)=>s+p.amount,0)
      const pendingCount  = (u.posts||[]).filter((p)=>p.status==="pending").length
      const profileLinks  = (linksData||[]).filter((l)=>l.user_id===u.id)
      return { ...u, views:v, byP, totalEarnings, openEarnings, payoutAmount, totalPaid, lastPayout, userPayouts, pendingCount, profileLinks }
      ...
      <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:60 }}>
        ...
        <div style={{ padding:"24px 18px 0" }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
            ...
            <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:8, display:"flex", alignItems:"center", gap:5 }}><Link size={11}/> Accounts</div>
            {(!u.profileLinks||u.profileLinks.length===0)&&<div style={{ fontSize:13, color:C.silver }}>Keine Accounts hinterlegt</div>}
            {(u.profileLinks||[]).map((l)=>{
              const p=PLATFORMS[l.platform]||{ label:l.platform, color:C.silver }
              return (
                <div key={l.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"5px 0" }}>
                  <span style={{ fontSize:10, color:p.color, fontWeight:700, textTransform:"uppercase", flex:"0 0 70px" }}>{p.label}</span>
                  <a href={l.url} target="_blank" rel="noreferrer" style={{ fontSize:12, color:C.blueGlow, wordBreak:"break-all" }}>{l.url}</a>
                </div>
              )
            })}
          </div>
          ...
          {u.payoutAmount>0&&(
            <div style={{ background:C.card, border:`1px solid ${C.green}40`, borderRadius:12, padding:"16px", marginBottom:12 }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                <span style={{ fontSize:13, color:C.silver }}>Auszahlbarer Betrag (approved)</span>
                <span style={{ fontSize:22, fontWeight:800, fontFamily:"'Space Grotesk',sans-serif", color:C.green }}>{eur(u.payoutAmount)}</span>
              </div>
              <input style={{ ...INPUT, marginBottom:10 }} placeholder="Notiz (optional)" value={payoutNote} onChange={(e)=>setPayoutNote(e.target.value)}/>
              <button onClick={()=>executePayout({...u, openEarnings:u.payoutAmount})} disabled={payoutBusy} style={{ width:"100%", background:payoutBusy?C.surface:`linear-gradient(135deg,#1a7a4a,${C.green})`, color:payoutBusy?C.silver:"#0B0B0F", border:"none", borderRadius:10, fontWeight:800, fontSize:14, padding:"13px 0", cursor:payoutBusy?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                <CheckCircle size={16}/> {payoutBusy?"Verarbeitung…":`Payout ${eur(u.payoutAmount)} bestätigen`}
              </button>
            </div>
          )}
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:100 }}>
      ...
      {tab==="dashboard"&&!isAdmin&&(
        <div style={{ padding:"24px 18px 0" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:16 }}>
            ...
            <KPI label="Aktive Videos" value={myActivePosts.length} sub={`von ${myPosts.length} eingereicht`} accent={C.green}/>
          ...
          {myPosts.length>0&&(
            <div>
              ...
              {[...myPosts].sort((a,b)=>latestV(b)-latestV(a)).map((p)=>{
                ...
                const lv = isRejected ? 0 : latestV(p)
                ...
                return (
                  <div key={p.id} style={{ background:C.card, border:`1px solid ${p.status==="rejected"?C.red+"30":C.border}`, borderLeft:`3px solid ${p.status==="pending"?C.orange:p.status==="rejected"?C.red:plat.color}`, borderRadius:10, padding:"12px 14px", marginBottom:6, display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <div style={{ textAlign:"right", flexShrink:0 }}>
                      <div style={{ fontSize:15, fontWeight:700, color:isRejected?C.silver:C.white }}>{isRejected?"—":fmt(lv)}</div>
                      <div style={{ fontSize:11, color:!isRejected&&earn>0?C.green:C.silver, fontWeight:600 }}>{!isRejected&&earn>0?eur(earn):"—"}</div>
                    </div>
                  </div>
                )
              })}
                ...
                <div key={post.id} style={{ background:C.card, border:`1px solid ${post.status==="rejected"?C.red+"30":C.border}`, borderLeft:`3px solid ${post.status==="pending"?C.orange:post.status==="rejected"?C.red:plat.color}`, borderRadius:12, marginBottom:8, overflow:"hidden", animation:"fadeUp .2s ease" }}>
                  <div onClick={()=>setExpanded(isOpen?null:post.id)} style={{ padding:"14px 16px", display:"flex", alignItems:"center", gap:12, cursor:"pointer" }}>
                    ...
                    <div style={{ textAlign:"right", flexShrink:0 }}>
                      ...
                      <div style={{ fontSize:12, color:post.status!=="rejected"&&earned>0?C.green:C.silver, fontWeight:600 }}>{post.status!=="rejected"&&earned>0?eur(earned):"—"}</div>
                    </div>
      ...
      {tab==="profile"&&(
        <div style={{ padding:"24px 18px 0", maxWidth:420 }}>
          ...
          {!isAdmin&&(
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"18px 16px", marginBottom:12 }}>
              <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:12, display:"flex", alignItems:"center", gap:5 }}><Link size={11}/>Meine Accounts</div>

              {/* Vorhandene Links */}
              {profileLinks.map((l)=>{
                const p = PLATFORMS[l.platform]||{ label:l.platform, color:C.silver }
                return (
                  <div key={l.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 0", borderBottom:`1px solid ${C.border}` }}>
                    <span style={{ fontSize:10, color:p.color, fontWeight:700, textTransform:"uppercase", flex:"0 0 70px" }}>{p.label}</span>
                    <a href={l.url} target="_blank" rel="noreferrer" style={{ flex:1, fontSize:12, color:C.blueGlow, wordBreak:"break-all", minWidth:0 }}>{l.url}</a>
                    <button onClick={()=>delProfileLink(l.id)} style={{ background:"none", border:"none", color:C.red+"80", cursor:"pointer", flexShrink:0, padding:4 }}><X size={13}/></button>
                  </div>
                )
              })}
              ...
              <div style={{ marginTop:12 }}>
                <div style={{ fontSize:11, color:C.silver, marginBottom:6 }}>Account hinzufügen</div>
                <div style={{ display:"flex", gap:6, marginBottom:6 }}>
                  {Object.entries(PLATFORMS).map(([k,p])=>(
                    <button key={k} onClick={()=>setNewLinkPlatform(k)} style={{ flex:1, padding:"7px 0", borderRadius:8, fontSize:11, fontWeight:600, border:`1px solid ${newLinkPlatform===k?p.color:C.border}`, background:newLinkPlatform===k?p.color+"18":C.surface, color:newLinkPlatform===k?p.color:C.silver, cursor:"pointer" }}>{p.label}</button>
                  ))}
                </div>
                <div style={{ display:"flex", gap:6 }}>
                  <input style={{ ...INPUT, marginBottom:0, flex:1 }} placeholder={`${PLATFORMS[newLinkPlatform]?.label} Profillink`} value={newLinkUrl} onChange={(e)=>setNewLinkUrl(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&addProfileLink()}/>
                  <button onClick={addProfileLink} style={{ background:`linear-gradient(135deg,${C.blue},${C.blueGlow})`, border:"none", borderRadius:8, color:"#fff", fontWeight:700, fontSize:13, padding:"0 14px", cursor:"pointer", flexShrink:0 }}><Plus size={16}/></button>
                </div>
                {linkMsg&&<div style={{ fontSize:12, color:linkMsg.includes("✓")?C.green:C.red, marginTop:6 }}>{linkMsg}</div>}
              </div>
            </div>
          )}
          ...
          {!isAdmin&&(
            <div style={{ marginTop:24 }}>
              {!showDeleteAccount&&(
                <button onClick={()=>setShowDeleteAccount(true)} style={{ width:"100%", background:"none", border:`1px solid ${C.red}40`, borderRadius:10, color:C.red+"90", fontSize:13, fontWeight:600, padding:"12px 0", cursor:"pointer" }}>
                  Account löschen
                </button>
              )}
              {showDeleteAccount&&(
                <div style={{ background:C.card, border:`1px solid ${C.red}50`, borderRadius:12, padding:"18px 16px" }}>
                  <div style={{ fontSize:14, fontWeight:700, color:C.red, marginBottom:6 }}>Account wirklich löschen?</div>
                  <div style={{ fontSize:12.5, color:C.silver, marginBottom:14 }}>Alle deine Daten, Posts und Views werden unwiderruflich gelöscht. Bitte Passwort zur Bestätigung eingeben.</div>
                  <input type="password" style={{ ...INPUT, marginBottom:8 }} placeholder="Passwort bestätigen" value={deletePassword} onChange={(e)=>setDeletePassword(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&deleteAccount()}/>
                  {deleteErr&&<div style={{ fontSize:12.5, color:C.red, marginBottom:8 }}>{deleteErr}</div>}
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={()=>{ setShowDeleteAccount(false); setDeletePassword(""); setDeleteErr("") }} style={{ flex:1, background:C.surface, border:`1px solid ${C.border}`, borderRadius:9, color:C.silver, fontSize:13, fontWeight:600, padding:"11px 0", cursor:"pointer" }}>
                      Abbrechen
                    </button>
                    <button onClick={deleteAccount} disabled={deleteBusy} style={{ flex:1, background:C.red, border:"none", borderRadius:9, color:"#fff", fontSize:13, fontWeight:700, padding:"11px 0", cursor:"pointer", opacity:deleteBusy?0.7:1 }}>
