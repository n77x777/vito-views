...
import { Plus, Trash2, X, ChevronDown, ChevronUp, LogOut, Shield, TrendingUp, Search, ArrowLeft, Wallet, User, Save } from "lucide-react"
...
export default function App() {
  ...
  const saveProfile = async () => {
    ...
    try {
      const updates = { username: profileForm.username.trim(), wallet: profileForm.wallet.trim() }
      const { error } = await supabase.from("profiles").update(updates).eq("id", session.user.id)
      if (error) { setProfileMsg("Fehler beim Speichern."); return }
  ...
  const adminTotals = useMemo(() => {
    ...
    return { totalUsers:allUsers.length, totalPosts:allUsers.reduce((s,u)=>s+(u.posts?.length||0),0), totalViews:allUsers.reduce((s,u)=>s+u.views,0), totalEarnings:allUsers.reduce((s,u)=>s+u.earnings,0) }
  }, [allUsers])
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:100 }}>
      ...
      {!isAdmin&&tab!=="profile"&&(
        <div style={{ padding:"28px 18px 16px" }}>
          ...
          <div style={{ fontSize:44, fontWeight:700, fontFamily:"'Space Grotesk',sans-serif", background:`linear-gradient(90deg,${C.white},${C.silverHi},${C.white})`, backgroundSize:"200% auto", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", animation:"shimmer 4s linear infinite" }}>{eur(totals.gE)}</div>
      ...
      {tab==="profile"&&(
        <div style={{ padding:"24px 18px 0", maxWidth:420 }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"18px 16px", marginBottom:12 }}>
            ...
            <div style={{ fontSize:11, color:C.silver, marginBottom:4, marginTop:4 }}>Neues Passwort (leer lassen = nicht ändern)</div>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"18px 16px", marginBottom:16 }}>
            ...
            <div style={{ fontSize:11, color:C.silver, marginTop:4 }}>Hier erhältst du deine Auszahlungen. Stelle sicher dass die Adresse korrekt ist.</div>
          </div>
          ...
          {Array.isArray(allUsers)&&(
            <div style={{ marginBottom:20 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:8 }}>
                {[{ label:"Nutzer", value:adminTotals.totalUsers, accent:C.silverHi },{ label:"Posts", value:adminTotals.totalPosts, accent:C.silver },{ label:"Views gesamt", value:fmt(adminTotals.totalViews), accent:C.blueGlow },{ label:"Payout gesamt", value:eur(adminTotals.totalEarnings), accent:C.green }].map((s)=>(
          ...
          {Array.isArray(allUsers)&&filteredUsers.map((row,i)=>(
            ...
            >
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:11, color:C.silver+"70", marginTop:3, display:"flex", alignItems:"center", gap:4 }}>
                  <Wallet size={10}/> {row.wallet?(row.wallet.length>24?row.wallet.slice(0,24)+"…":row.wallet):"Keine Wallet eingetragen"}
                </div>
              </div>
              <div style={{ textAlign:"right", flexShrink:0 }}>
                <div style={{ fontSize:18, fontWeight:700, fontFamily:"'Space Grotesk',sans-serif", color:row.earnings>0?C.green:C.silver }}>{eur(row.earnings)}</div>
