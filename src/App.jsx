...
export default function App() {
  ...
  const handleAuth = async () => {
    ...
    try {
      if (authMode==="signup") {
        ...
        if (data.user) await supabase.from("profiles").insert({ id:data.user.id, username:form.username.trim().toLowerCase(), wallet:form.wallet.trim(), profile_link:form.profile_link.trim() })
  ...
  const addPost = async () => {
    ...
    // Profillink-Check für User (nicht Admin)
  ...
  const setPostStatus = async (postId, status) => {
    ...
    if (selUser) {
      setSelUser((u)=>({ ...u, posts: u.posts.map((p)=>p.id===postId?{...p,status}:p) }))
    }
    setStatusBusy(null)
  }
                ...
                return (
                  <div key={p.id} style={{ background:C.card, border:`1px solid ${p.status==="rejected"?C.red+"30":C.border}`, borderLeft:`3px solid ${p.status==="pending"?C.orange:p.status==="rejected"?C.red:plat.color}`, borderRadius:10, padding:"12px 14px", marginBottom:6, display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <div style={{ textAlign:"right", flexShrink:0 }}>
                      ...
                      <div style={{ fontSize:11, color:p.status==="approved"&&earn>0?C.green:C.silver, fontWeight:600 }}>{p.status==="approved"&&earn>0?eur(earn):p.status==="rejected"?"—":"—"}</div>
                    </div>
                  </div>
                )
              })}
