...
import { Plus, Trash2, X, ChevronDown, ChevronUp, LogOut, Shield, TrendingUp, Search, ArrowLeft, Wallet, User, Save, CheckCircle, Clock } from "lucide-react"
...
const ADMIN_EMAILS = ["nickarapsanov@yahoo.com", "alexeyzh2007@gmail.com"]
...
export default function App() {
  ...
  const [profileForm, setProfileForm] = useState({ username:"", wallet:"", newPassword:"" })
  ...
  const fetchProfile = async (userId) => {
    ...
    setProfile(data)
    setProfileForm({ username: data?.username||"", wallet: data?.wallet||"", newPassword:"" })
    setLoading(false)
    setScreen("app")
    if (data?.is_admin) setTab("admin")
    else { setTab("posts"); fetchPosts(userId); fetchMyPayouts(userId) }
  }
    ...
    try {
      if (authMode === "signup") {
        ...
        if (data.user) {
          ...
          await supabase.from("profiles").insert({ id: data.user.id, username: form.username.trim().toLowerCase(), wallet: form.wallet.trim(), is_admin: isAdmin })
        }
  ...
  const logout = async () => {
    ...
    setPosts([]); setBoard(null); setAllUsers(null); setSelUser(null); setMyPayouts([])
  ...
  const saveProfile = async () => {
    ...
    try {
      await supabase.from("profiles").update({ username: profileForm.username.trim(), wallet: profileForm.wallet.trim() }).eq("id", session.user.id)
      if (profileForm.newPassword) await supabase.auth.updateUser({ password: profileForm.newPassword })
      await fetchProfile(session.user.id)
      setProfileMsg("Gespeichert ✓")
    } catch { setProfileMsg("Fehler beim Speichern.") }
    finally { setBusy(false) }
  }

  const fetchPosts = async (userId) => {
    const { data } = await supabase.from("posts").select("*, view_entries(*)").eq("user_id", userId).order("created_at", { ascending: false })
    setPosts(data || [])
  }
  ...
  // Admin löscht Post eines anderen Users
  const adminDelPost = async (postId, userId) => {
    if (!window.confirm("Diesen Post wirklich löschen?")) return
    await supabase.from("view_entries").delete().eq("post_id", postId)
    await supabase.from("posts").delete().eq("id", postId)
    // Refresh selUser
    await loadAllUsers()
    setSelUser((prev) => prev ? { ...prev, posts: prev.posts.filter((p)=>p.id!==postId) } : prev)
  }

  // Admin löscht ganzen User-Account
  const adminDelUser = async (user) => {
    if (!window.confirm(`Nutzer "${user.username}" und alle seine Daten wirklich löschen?\nDas kann nicht rückgängig gemacht werden.`)) return
    // Posts + Entries + Payouts werden durch CASCADE gelöscht
  ...
  const calcTotalEarnings = (userPosts) => {
    let total = 0
    userPosts?.forEach((p) => { total += payout(latestV(p)) })
    return total
  }

  const executePayout = async (user) => {
    if (!window.confirm(`Payout für ${user.username} bestätigen?\nBetrag: ${eur(user.openEarnings)}\nWallet: ${user.wallet||"—"}`)) return
    setPayoutBusy(user.id)
    try {
      await supabase.from("payouts").insert({ user_id: user.id, amount: user.openEarnings, note: payoutNote||null, paid_at: new Date().toISOString() })
      await loadAllUsers()
      setPayoutNote("")
  ...
  const totals = useMemo(() => {
    const byP = { tiktok:0, instagram:0 }; let gV=0, gE=0
    ...
    const openEarnings = calcOpenEarnings(posts, lastPayout?.paid_at?.slice(0,10)||null)
    return { byP, gV, gE, openEarnings, lastPayout }
  }, [posts, myPayouts])
  ...
  const loadBoard = async () => {
    setBoard("loading")
    const { data } = await supabase.from("profiles").select("username, posts(*, view_entries(*))").eq("is_admin", false)
    const rows = (data||[]).map((u) => { let v=0,e=0; u.posts?.forEach((p)=>{ const lv=latestV(p); v+=lv; e+=payout(lv) }); return { username:u.username, views:v, earnings:e, count:u.posts?.length||0 } }).sort((a,b)=>b.earnings-a.earnings)
    setBoard(rows)
  }

  const loadAllUsers = async () => {
    setAllUsers("loading")
    const { data: usersData } = await supabase.from("profiles").select("*, posts(*, view_entries(*))").eq("is_admin", false)
    ...
    const rows = (usersData||[]).map((u) => {
      ...
      const openEarnings = calcOpenEarnings(u.posts, lastPayout?.paid_at?.slice(0,10)||null)
  ...
  useEffect(() => { if(tab==="board") loadBoard(); if(tab==="admin") loadAllUsers() }, [tab])
      ...
      <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:80 }}>
        ...
        <div style={{ padding:"24px 18px 0" }}>
          <div style={{ display:"flex", gap:10, overflowX:"auto", paddingBottom:4, marginBottom:16 }}>
            <StatCard label="Offen" value={eur(u.openEarnings)} sub="seit letztem Payout" accent={C.green}/>
            <StatCard label="Gesamt" value={eur(u.totalEarnings)} accent={C.blueGlow}/>
            <StatCard label="Gezahlt" value={eur(u.totalPaid)} accent={C.silver}/>
            <StatCard label="Views" value={fmt(u.views)} accent={C.silverHi}/>
          </div>

          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:8, display:"flex", alignItems:"center", gap:5 }}><Wallet size={11}/> Wallet-Adresse</div>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
            ...
            {u.lastPayout ? <div><div style={{ fontSize:16, fontWeight:700, color:C.green }}>{eur(u.lastPayout.amount)}</div><div style={{ fontSize:12, color:C.silver, marginTop:3 }}>{fmtDate(u.lastPayout.paid_at)}{u.lastPayout.note&&` · ${u.lastPayout.note}`}</div></div>
            : <div style={{ fontSize:13, color:C.silver }}>Noch kein Payout</div>}
          </div>
          ...
          {u.openEarnings > 0 && (
            <div style={{ background:C.card, border:`1px solid ${C.green}40`, borderRadius:12, padding:"16px", marginBottom:12 }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                <div style={{ fontSize:13, color:C.silver }}>Offener Betrag</div>
                <div style={{ fontSize:22, fontWeight:800, fontFamily:"'Space Grotesk',sans-serif", color:C.green }}>{eur(u.openEarnings)}</div>
              </div>
              <input style={{ ...INPUT, marginBottom:10 }} placeholder="Notiz (optional, z.B. Transaktions-ID)" value={payoutNote} onChange={(e)=>setPayoutNote(e.target.value)}/>
              <button onClick={()=>executePayout(u)} disabled={payoutBusy===u.id} style={{ width:"100%", background:`linear-gradient(135deg,#1a7a4a,${C.green})`, color:"#0B0B0F", border:"none", borderRadius:10, fontWeight:800, fontSize:14, padding:"13px 0", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, opacity:payoutBusy===u.id?0.7:1 }}>
                <CheckCircle size={16}/> {payoutBusy===u.id?`Verarbeitung…`:`Payout ${eur(u.openEarnings)} bestätigen`}
              </button>
            </div>
          )}
          ...
          <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:10 }}>Posts</div>
          {(!u.posts||u.posts.length===0)&&<div style={{ color:C.silver, fontSize:13, marginBottom:16 }}>Keine Posts.</div>}
          {[...(u.posts||[])].sort((a,b)=>latestV(b)-latestV(a)).map((p)=>{
            const plat=PLATFORMS[p.platform]; const lv=latestV(p); const earn=payout(lv)
            return (
              <div key={p.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderLeft:`3px solid ${plat.color}`, borderRadius:12, padding:"14px 16px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div style={{ minWidth:0, flex:1 }}>
                  ...
                  {p.url&&<a href={p.url} target="_blank" rel="noreferrer" style={{ fontSize:11, color:C.blueGlow, display:"block", marginTop:2, wordBreak:"break-all" }}>{p.url}</a>}
                </div>
                <div style={{ textAlign:"right", flexShrink:0, marginLeft:12 }}>
                  ...
                  <button onClick={()=>adminDelPost(p.id, u.id)} style={{ display:"flex", alignItems:"center", gap:4, background:"none", border:"none", color:C.red+"80", fontSize:11, cursor:"pointer", padding:0, marginTop:4, marginLeft:"auto" }}><Trash2 size={11}/> Löschen</button>
                </div>
              </div>
            )
          })}
          ...
          <div style={{ marginTop:24, borderTop:`1px solid ${C.border}`, paddingTop:20 }}>
            <div style={{ fontSize:10, color:C.red, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:10 }}>Gefahrenzone</div>
            <button onClick={()=>adminDelUser(u)} style={{ width:"100%", background:"transparent", border:`1px solid ${C.red}60`, borderRadius:10, color:C.red, fontWeight:700, fontSize:13, padding:"12px 0", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              <Trash2 size={14}/> Nutzer "{u.username}" komplett löschen
            </button>
          </div>
        </div>
      </div>
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:100 }}>
      ...
      {!isAdmin&&tab==="posts"&&(
        <div style={{ padding:"28px 18px 16px" }}>
          <div style={{ fontSize:11, color:C.silver, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:4 }}>Offen seit letztem Payout</div>
          <div style={{ fontSize:44, fontWeight:700, fontFamily:"'Space Grotesk',sans-serif", background:`linear-gradient(90deg,${C.white},${C.silverHi},${C.white})`, backgroundSize:"200% auto", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", animation:"shimmer 4s linear infinite" }}>{eur(totals.openEarnings)}</div>
          <div style={{ fontSize:13, color:C.silver, marginTop:4 }}>Gesamt verdient: {eur(totals.gE)} · {fmt(totals.gV)} Views</div>
          {totals.lastPayout&&<div style={{ fontSize:12, color:C.silver, marginTop:4, display:"flex", alignItems:"center", gap:5 }}><CheckCircle size={12} color={C.green}/> Letzter Payout: {eur(totals.lastPayout.amount)} am {fmtDate(totals.lastPayout.paid_at)}</div>}
          <div style={{ display:"flex", gap:10, marginTop:20, overflowX:"auto", paddingBottom:4 }}>
            ...
            <StatCard label="Auszahlbar" value={eur(totals.openEarnings)} accent={C.green}/>
          </div>
        </div>
      )}
      {isAdmin&&tab==="admin"&&(
      ...
      <div style={{ display:"flex", padding:"0 18px", borderBottom:`1px solid ${C.border}`, overflowX:"auto" }}>
        {TABS.map((t)=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{ padding:"12px 16px", background:"none", border:"none", borderBottom:`2px solid ${tab===t.id?C.blueGlow:"transparent"}`, color:tab===t.id?C.white:C.silver, fontWeight:tab===t.id?700:500, fontSize:13.5, cursor:"pointer", whiteSpace:"nowrap" }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab==="profile"&&(
        <div style={{ padding:"24px 18px 0", maxWidth:420 }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"18px 16px", marginBottom:12 }}>
            <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:12 }}>Account-Daten</div>
            <div style={{ fontSize:11, color:C.silver, marginBottom:4 }}>Benutzername</div>
            <input style={INPUT} placeholder="Benutzername" value={profileForm.username} onChange={(e)=>setProfileForm((f)=>({...f,username:e.target.value}))}/>
          ...
          {myPayouts.length > 0 && (
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"18px 16px", marginBottom:12 }}>
              <div style={{ fontSize:10, color:C.silver, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:12, display:"flex", alignItems:"center", gap:5 }}><Clock size={11}/>Meine Payout-History</div>
              {myPayouts.map((p)=>(
      ...
      {tab==="posts"&&(
        <>
          ...
          <div style={{ padding:"12px 18px 0" }}>
            {filtered.length===0&&<div style={{ textAlign:"center", padding:"70px 20px" }}><TrendingUp size={28} color={C.border} style={{ marginBottom:12 }}/><div style={{ color:C.silver, fontSize:14 }}>Noch keine Posts erfasst.</div></div>}
            {filtered.map((post)=>{
                        ...
                        {entries.map((h,i)=>{ const pv=i>0?entries[i-1].views:null; const d=pv!==null?h.views-pv:null; return (
                          <div key={h.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"7px 0", borderBottom:i<entries.length-1?`1px solid ${C.border}`:"none" }}>
                            ...
                            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                              ...
                              <button onClick={()=>delEntry(h.id)} style={{ background:"none", border:"none", color:C.silver+"50", cursor:"pointer", padding:2 }}><X size={12}/></button>
                            </div>
                          </div>
                        )})}
                      </div>
      ...
      {tab==="board"&&(
        <div style={{ padding:"16px 18px 0" }}>
          ...
          {Array.isArray(board)&&board.length===0&&<div style={{ color:C.silver, fontSize:13, padding:24 }}>Noch keine Nutzer.</div>}
      ...
      {tab==="admin"&&isAdmin&&(
        <div style={{ padding:"16px 18px 0" }}>
          {Array.isArray(allUsers)&&(
            <div style={{ marginBottom:20 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:8 }}>
                {[{ label:"Nutzer", value:adminTotals.totalUsers, accent:C.silverHi },{ label:"Posts", value:adminTotals.totalPosts, accent:C.silver },{ label:"Views gesamt", value:fmt(adminTotals.totalViews), accent:C.blueGlow },{ label:"Bereits gezahlt", value:eur(adminTotals.totalPaid), accent:C.silver }].map((s)=>(
              ...
              <div style={{ background:C.card, border:`1px solid ${C.green}40`, borderRadius:10, padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <div><div style={{ fontSize:10, color:C.green, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase" }}>Ausstehende Auszahlungen</div><div style={{ fontSize:11.5, color:C.silver, marginTop:2 }}>Summe aller offenen Beträge</div></div>
          ...
          {Array.isArray(allUsers)&&filteredUsers.length===0&&<div style={{ color:C.silver, fontSize:13, padding:"12px 0" }}>Keine Nutzer gefunden.</div>}
          {Array.isArray(allUsers)&&filteredUsers.map((row,i)=>(
            <div key={row.id} onClick={()=>setSelUser(row)} style={{ background:C.card, border:`1px solid ${row.openEarnings>0?C.green+"40":C.border}`, borderRadius:12, marginBottom:8, padding:"14px 16px", cursor:"pointer", display:"flex", alignItems:"center", gap:12 }}
            ...
            >
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:11, color:C.silver+"70", marginTop:2, display:"flex", alignItems:"center", gap:4 }}><Wallet size={10}/> {row.wallet?(row.wallet.length>24?row.wallet.slice(0,24)+"…":row.wallet):"Keine Wallet"}</div>
