...
export default function App() {
  ...
  const fetchProfile = async (userId) => {
    ...
    // Admins immer Posts-Tab zeigen, damit sie eigene Posts anlegen können
    setTab("posts")
    fetchPosts(userId)
  ...
  // ── Posts: Admin lädt ALLE Posts, User nur eigene ──
  ...
  // Nach dem fetchProfile ist profile noch nicht gesetzt beim ersten load — 
  // deswegen nochmal laden wenn profile sich ändert
  useEffect(() => {
    if (session && profile !== null) fetchPosts(session.user.id)
  }, [profile?.is_admin])
  ...
  // Admin kann jeden Post löschen, User nur eigene
  ...
  const calcTotalEarnings = (userPosts) => {
    let total = 0; userPosts?.forEach((p) => { total += payout(latestV(p)) }); return total
  }
  ...
  const executePayout = async (user) => {
    ...
    try {
      const { error } = await supabase.from("payouts").insert({
        user_id: user.id, amount, note: payoutNote||null, paid_at: new Date().toISOString()
      })
  ...
  // Meine eigenen Posts (für Verdienst-Anzeige)
  ...
  const totals = useMemo(() => {
    ...
    myPosts.forEach((p) => { const v=latestV(p); byP[p.platform]=(byP[p.platform]||0)+v; gV+=v; gE+=payout(v) })
  ...
  const loadBoard = async () => {
    ...
    const rows = (data||[]).map((u) => {
      let v=0,e=0; u.posts?.forEach((p)=>{ const lv=latestV(p); v+=lv; e+=payout(lv) })
      ...
      <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:60 }}>
        ...
        <div style={{ padding:"24px 18px 0" }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
            ...
            {u.profile_link ? <a href={u.profile_link} target="_blank" rel="noreferrer" style={{ fontSize:13, color:C.blueGlow, wordBreak:"break-all" }}>{u.profile_link}</a> : <div style={{ fontSize:13, color:C.silver }}>—</div>}
          </div>
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
            ...
            return (
              <div key={p.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderLeft:`3px solid ${plat.color}`, borderRadius:12, padding:"14px 16px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:12, flexShrink:0, marginLeft:12 }}>
                  ...
                  <button onClick={()=>delPost(p.id)} style={{ background:"none", border:"none", color:C.red+"70", cursor:"pointer", padding:4 }}><Trash2 size={15}/></button>
                </div>
              </div>
            )
          })}
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:100 }}>
      ...
      {tab==="posts"&&(
        <div style={{ padding:"28px 18px 16px" }}>
          <div style={{ fontSize:11, color:C.silver, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:4 }}>
            {isAdmin ? "Meine Posts als Admin" : "Offen seit letztem Payout"}
          </div>
          {!isAdmin&&<>
                ...
                <div key={post.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, marginBottom:8, overflow:"hidden", animation:"fadeUp .2s ease" }}>
                  ...
                  {isOpen&&(
                    <div style={{ padding:"0 16px 16px", borderTop:`1px solid ${C.border}` }}>
                      ...
                      {/* Admin oder Besitzer kann löschen */}
                      {(isAdmin||isOwnPost)&&(
